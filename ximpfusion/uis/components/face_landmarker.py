...
from ximpfusion import face_landmarker, state_manager, wording
from ximpfusion.common_helper import calc_float_step
from ximpfusion.typing import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = ximpfusion.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(ximpfusion.choices.face_landmarker_score_range),
		minimum = ximpfusion.choices.face_landmarker_score_range[0],
		maximum = ximpfusion.choices.face_landmarker_score_range[-1]
	)
